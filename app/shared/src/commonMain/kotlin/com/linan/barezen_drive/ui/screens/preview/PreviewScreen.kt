 ...
 * favorite, archive and delete for the item currently in view. The album is
 * behind a backstack entry, so returning to it (which [onBack] does right
 * after an archive or delete) reloads the list and the change is visible.
 */
@Composable
fun PreviewScreen(
    files: List<FileDto>,
    ...
    // Optimistic flag state so the icons react instantly; a failed request
    // rolls the flag back and reports through the snackbar.
    var favorited by remember(file.id) { mutableStateOf(file.isFavorite) }
            ...
            if (showActions) {
                Row(
                    ...
                    IconButton(onClick = {
                        ...
                        favorited = next
                        scope.launch {
                            repo.setFavorite(file.id, next).onFailure {
                                favorited = !next
                                snackbar.showSnackbar(I18n.strings.operationFailed)
                            }
                        }
                    }) {
                        Icon(
                            if (favorited) Icons.Default.Favorite else Icons.Default.FavoriteBorder,
                            contentDescription = if (favorited) LocalStrings.current.actionUnfavorite
                    ...
                    IconButton(onClick = {
                        ...
                        Icon(
                            ...
                            contentDescription = if (archived) LocalStrings.current.actionUnarchive
