...
- Media library management: favourites, archive and a 30-day trash, all stored as
  flags on the file row, plus a per-album auto backup queue on Android (MediaStore
  observer, persistent SQLite queue, foreground worker, per-album inclusion).
- File version history: an overwrite upload keeps the replaced content as a
  restorable revision (up to 20 per file); versions share blob reference counting
  with live files, so the same content is still stored once.
- Object storage backend: `STORAGE_BACKEND=s3` moves content into any S3-compatible
  bucket (MinIO, R2, COS) using a self-implemented SigV4 signer, no AWS SDK.
- Home dashboard: `GET /api/server/stats` (authenticated) reports CPU, memory,
  disk, network throughput and uptime.
