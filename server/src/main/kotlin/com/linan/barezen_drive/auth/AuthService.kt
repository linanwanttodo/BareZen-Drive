...
object AuthService {
    ...
    fun issueTokens(uid: UUID): LoginResponse = transaction(DatabaseFactory.db) {
        // Single transaction: mint + persist + read the user in one atomic step.
        // When called from login() this reuses the outer transaction (Exposed
        // nests via the thread-local), so the token row and the user read can
        ...
        LoginResponse(JwtService.issue(uid.toString()), raw, user)
    }
