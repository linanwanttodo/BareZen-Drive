...
import com.linan.barezen_drive.files.trashRoutes
import com.linan.barezen_drive.files.uploadRoutes
import com.linan.barezen_drive.files.versionRoutes
...
fun main() {
    ...
    val storage: StorageProvider = when (cfg.storageBackend) {
        ...
        else -> LocalStorageProvider(java.nio.file.Path.of(cfg.storageDir))
    }
...
fun Application.module(cfg: AppConfig, storage: StorageProvider) {
    ...
    // The storage registry is the reserved seam for a multi-node deployment: the
    // active backend is registered under its id, and every key stored today is
    // bare so it resolves to this one provider (see StorageRegistry).
    StorageRegistry.register(cfg.storageBackendId, storage)
    // The throttle is an in-process singleton; reset it on (re)start so a fresh
