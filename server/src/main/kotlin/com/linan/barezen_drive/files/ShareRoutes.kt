...
fun Route.sharePublicRoutes(storage: StorageProvider) {
    route("/api/public/shares/{token}") {
        get {
            ...
            withContext(Dispatchers.IO) { ShareService.recordView(share, call.clientIp()) }
            call.respond(info)
        }
        ...
        get("/files/{fid}/thumbnail") {
            ...
            val bytes = withContext(Dispatchers.IO) {
                if (!storage.exists(key)) throw com.linan.barezen_drive.api.ApiException.notFound("缩略图不存在")
