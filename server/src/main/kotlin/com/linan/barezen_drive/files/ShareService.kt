...
object ShareService {
    ...
    /** Bumps the view counter for a share landing/listing request. De-duplicated
     *  per source: one visitor refreshing the link must not write the DB every
     *  time nor inflate viewCount, so repeats within 5 minutes are collapsed. */
    fun recordView(share: ResolvedShare, source: String) {
    ...
    fun sharedFileMeta(share: ResolvedShare, fileId: UUID): FileMeta = transaction(DatabaseFactory.db) {
        val row = FilesTable.selectAll().where {
            (FilesTable.id eq fileId) and (FilesTable.deletedAt eq 0L)
        }.singleOrNull()
            ?: throw ApiException.notFound("文件不存在")
        if (share.fileId != null) {
