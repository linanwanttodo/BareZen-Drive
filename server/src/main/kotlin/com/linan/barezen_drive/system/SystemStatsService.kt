...
object SystemStatsService {
    ...
    private fun readProcNet(): Pair<Long, Long> {
        ...
        File("/proc/net/dev").bufferedReader().forEachLine { line ->
            val parts = line.split(':')
            // Exclude only the loopback interface by exact name. endsWith("lo")
            // would also drop a real interface whose name happens to end in lo.
            if (parts.size == 2 && parts[0].trim() != "lo") {
