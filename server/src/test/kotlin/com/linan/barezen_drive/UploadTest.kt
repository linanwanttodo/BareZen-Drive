...
class UploadTest {
    ...
    fun renameMoveDeletePositive() = testApplication {
        ...
        // Delete A and B: both move to the trash and keep their blob alive. A
        // trashed row still counts as a reference - dropping that would let
        // purging one copy destroy bytes another trashed row points at.
        ...
        // Trash holds both; no live listing shows them any more.
        val trashed = client.get("/api/trash") { header(HttpHeaders.Authorization, auth) }
        assertEquals(HttpStatusCode.OK, trashed.status)
        assertEquals(setOf(aId, bId), json.decodeFromString<TrashResponse>(trashed.bodyAsText()).files.map { it.id }.toSet())
        val rootAfterTrash = json.decodeFromString<ContentsResponse>(client.get("/api/folders/root/contents") { header(HttpHeaders.Authorization, auth) }.bodyAsText())
        assertFalse(rootAfterTrash.files.any { it.name == "fileB.txt" }, "trashed file must leave the live listing")
        val destAfterTrash = json.decodeFromString<ContentsResponse>(client.get("/api/folders/$destId/contents") { header(HttpHeaders.Authorization, auth) }.bodyAsText())
        assertFalse(destAfterTrash.files.any { it.name == "renamedA.txt" }, "trashed file must leave its folder listing")
        // While both rows sit in the trash the blob is still referenced, so the
        // same content is an instant upload.
        val keptInstant = client.post("/api/uploads/init") { header(HttpHeaders.Authorization, auth); contentType(ContentType.Application.Json); setBody("""{"name":"fresh.txt","size":${body.size},"sha256":"$sha"}""") }
        assertEquals(HttpStatusCode.OK, keptInstant.status, keptInstant.bodyAsText())
        assertTrue(json.decodeFromString<UploadInitResponse>(keptInstant.bodyAsText()).instantUpload,
            "trashed rows must keep the blob referenced")
        // Emptying the trash drops the last references: the blob goes, so the next
        // init of the same content is a real upload again.
