...
class ShareLinkTest {
    ...
    fun viewAndDownloadCountersAccumulate() = testApplication {
        ...
        val token = tokenOf(createShare("""{"fileId":"$fileId"}""").url)
        // View counts are de-duplicated per source over a 5-minute window: two
        // landings from the same visitor collapse to one view, but a different
        // source is counted separately. Downloads are never de-duplicated.
        repeat(2) {
            assertEquals(HttpStatusCode.OK, client.get("/api/public/shares/$token") { header("X-Forwarded-For", "203.0.113.1") }.status)
        }
        assertEquals(HttpStatusCode.OK, client.get("/api/public/shares/$token") { header("X-Forwarded-For", "198.51.100.7") }.status)
        repeat(2) {
            assertEquals(HttpStatusCode.OK, client.get("/api/public/shares/$token/files/$fileId/content") { header("X-Forwarded-For", "203.0.113.1") }.status)
        }
        val listed = client.get("/api/shares?fileId=$fileId") { header(HttpHeaders.Authorization, auth) }
        val share = json.decodeFromString<com.linan.barezen_drive.core.dto.SharesResponse>(listed.bodyAsText()).shares.single()
        // 2 same-source landings -> 1, plus 1 distinct-source landing -> 1: total 2.
        assertEquals(2L, share.viewCount, "per-source 5-minute view de-duplication")
