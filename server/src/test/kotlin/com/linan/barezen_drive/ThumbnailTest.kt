...
class ThumbnailTest {
    ...
    @Test
    fun flaggedCoverWithMissingBlobAnswers404() = testApplication {
        setup()
        val body = "cover that outlives its blob".encodeToByteArray()
        val fileId = upload(body, "gone.jpg", mime = "image/jpeg")
        val put = client.put("/api/files/$fileId/thumbnail") {
            header(HttpHeaders.Authorization, auth); contentType(ContentType.Image.JPEG); setBody(fakeJpeg())
        }
        assertEquals(HttpStatusCode.OK, put.status, put.bodyAsText())
        // The row keeps claiming a cover, but the blob goes away behind its back -
        // what a manual store prune or a half-finished purge looks like. The
        // exists() guard must turn that into 404, not a 500 from FileInputStream.
        assertTrue(Files.deleteIfExists(java.nio.file.Path.of(storageDir, "thumbs", "${sha256hex(body)}.jpg")))
        val get = client.get("/api/files/$fileId/thumbnail") { header(HttpHeaders.Authorization, auth) }
        assertEquals(HttpStatusCode.NotFound, get.status, get.bodyAsText())
        assertTrue(get.bodyAsText().contains("NOT_FOUND"), get.bodyAsText())
    }
}
