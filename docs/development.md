...
默认内容落在 `STORAGE_DIR` 卷上。把 `.env` 里的 `STORAGE_BACKEND` 改为 `s3` 即可改存任意 S3 兼容桶（MinIO、R2、COS 等），无需额外依赖：服务端用 JDK 自带的 HTTP 客户端加自实现 SigV4 签名。

```bash
STORAGE_BACKEND=s3
S3_ENDPOINT=https://minio.example.com:9000   # 只写 scheme+host，不要带桶名
S3_BUCKET=barezen
S3_REGION=us-east-1
S3_ACCESS_KEY=...
S3_SECRET_KEY=...
S3_PATH_STYLE=true                            # MinIO 自建网关通常需要路径式寻址
```

要点：

- `STORAGE_DIR` 仍然必需：上传分块先在该目录暂存再合并，单请求体超过内存缓冲阈值时也用它做临时落盘。
- 启动时只校验配置（缺 `S3_BUCKET` 或凭据、`S3_ENDPOINT` 带了路径、path-style 未给 endpoint 都会启动失败并给出明确原因），不做联网探测；桶不存在或权限不足会在第一次写入时暴露。
- 切换到 s3 后端不会迁移已有的本地 blob，旧文件按旧 key 读取（`StorageRegistry` 未注册的 backend 直接报错，不会静默回退），因此混用前需自行同步数据。
- 版本 blob 与文件 blob 共用同一套引用计数，回收站与历史版本持有的字节不会被提前删除。
