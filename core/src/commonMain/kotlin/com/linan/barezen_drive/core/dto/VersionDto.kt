 ...
 * resolved (offline server, unreachable manifest) are null, and updateAvailable
...
@Serializable data class VersionInfoResponse(
    ...
    val apiVersion: Int,
    /** Newest upstream release version, without the leading "v"; null when unknown. */
    val latestVersion: String? = null,
    /** Human-facing release page URL; null when no release could be resolved. */
    val releaseUrl: String? = null,
    /** True only when latestVersion is known and strictly newer than serverVersion. */
    val updateAvailable: Boolean = false,
    /** Per-platform download packages from the release manifest. */
