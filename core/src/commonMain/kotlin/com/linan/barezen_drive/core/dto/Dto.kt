...
@Serializable data class FileDto(
    ...
    /** ISO-8601 archive time; null when not archived. */
...
@Serializable data class UploadInitRequest(val folderId: String? = null, val name: String, val size: Long, val mimeType: String? = null, val sha256: String? = null, val chunkSize: Long? = null, val takenAt: Long? = null, val overwrite: Boolean = false)
...
@Serializable data class UploadCompleteResponse(val file: FileDto, val replacedVersion: FileVersionDto? = null)

// ---- File versions (overwrite upload keeps history) ----

/** One superseded revision of a file. The live row always points at the newest
 *  content; every earlier content lands here with its own blob reference. */
@Serializable data class FileVersionDto(
    val id: String,
    val fileId: String,
    val revision: Long,
    val size: Long,
...
@Serializable data class FileVersionsResponse(val versions: List<FileVersionDto>)
...
/** Toggle favorite flag on a single file. */
@Serializable data class FavoriteRequest(val favorite: Boolean)
