...
- 服务端分块流式上传。
- 媒体库管理：收藏、归档与 30 天回收站，都只是文件行上的标记；Android 侧另有按相册勾选的自动备份
  队列（MediaStore 监听 + SQLite 持久队列 + 前台 worker 保活）。
- 文件版本历史：带覆盖的上传会把被替换的内容留成可恢复的历史版本（每个文件最多 20 版），版本与现行
  内容共用 blob 引用计数，同样内容仍只存一份。
- 对象存储后端：`STORAGE_BACKEND=s3` 可把内容改存任意 S3 兼容桶（MinIO、R2、COS），签名走自实现的
  SigV4，不引入 AWS SDK。
- 首页状态仪表盘：`GET /api/server/stats`（需登录）返回 CPU、内存、磁盘、上下行速率与 uptime。
